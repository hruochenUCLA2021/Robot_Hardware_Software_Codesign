#!/usr/bin/env python3
"""Real-time MuJoCo CPU sim for HERMES Hi joystick policies.

Supports both:
- `HiJoystick*` (actor obs includes local linvel)
- `HiJoystick*NoLinearVel` (actor obs excludes local linvel)

Policy loading uses Brax PPO checkpoint restore (same approach as the reference
`load_and_rollout_Hi_joystick*.py` scripts), but simulation is MuJoCo CPU with
viewer.
"""
...
def _add_reference_to_syspath(ws: Path) -> None:
    ...
    hermes_root = ref / "HERMES_simulation_model"
...
class RealtimeHermesHiPolicyNode(Node):
    def __init__(self) -> None:
        super().__init__("realtime_hermes_hi_policy")

        ws = _find_workspace_root()
        _add_reference_to_syspath(ws)

        # Params
        self.declare_parameter("env_name", "HiJoystickFlatTerrainNoLinearVel")
        self.declare_parameter(
            "checkpoint_dir",
            str(
                ws
                / "reference"
                / "HERMES_simulation_model"
                / "ModifyHighTorque_to_Playground"
                / "HiEnv"
                / "training"
                / "checkpoints"
                / "HiJoystickFlatTerrainNoLinearVel_flat"
                / "final"
            ),
        )
        self.declare_parameter("env_config_path", "")
        self.declare_parameter(
            "xml_path",
            str(
                ws
        ...
        self.declare_parameter("max_vy", 0.8)
        self.declare_parameter("max_wz", 1.0)
        self.declare_parameter("show_contact", False)

        env_name = str(self.get_parameter("env_name").value)
        ckpt_dir = Path(str(self.get_parameter("checkpoint_dir").value)).expanduser()
        env_config_path = str(self.get_parameter("env_config_path").value).strip()
        ...
        from ModifyHighTorque_to_Playground.HiEnv import registry as hi_registry  # type: ignore
        ...
        # --- Load env config + policy (JAX/Brax) ---
        EnvClass, default_config = hi_registry.get_hi_environment(env_name)
        ...
        if env_name in ("HiJoystickFlatTerrain", "HiJoystickFlatTerrainNoLinearVel"):
            task = "flat_terrain"
        elif env_name in ("HiJoystickRoughTerrain", "HiJoystickRoughTerrainNoLinearVel"):
            task = "rough_terrain"
        else:
            task = "flat_terrain"
        ...
        # Default pose from keyframe home if present.
        try:
            ...
            self._default_qpos = np.array(self._mj_model.qpos0, dtype=np.float32)
            self._default_pose = np.array(self._mj_model.qpos0[7:], dtype=np.float32)
        ...
        self._include_linvel = "NoLinearVel" not in env_name
        self._nu = int(self._mj_model.nu)
        self._nqj = int(self._mj_model.nq - 7)
        self._last_act = np.zeros((self._nu,), dtype=np.float32)

        # Realtime loop at ctrl_dt from env config (default 0.02).
        self._ctrl_dt = float(getattr(env_cfg, "ctrl_dt", 0.02))
        self._n_substeps = max(
            1, int(round(self._ctrl_dt / float(self._mj_model.opt.timestep)))
        )

        # Phase clock like envs (2 legs).
        self._phase = np.array([0.0, np.pi], dtype=np.float32)
        gait_freq = 1.5  # fixed for realtime; training randomizes
        ...
        self.get_logger().info(f"env_name={env_name} include_linvel={self._include_linvel}")
        self.get_logger().info(f"xml={xml_path}")
        self.get_logger().info(f"ckpt={ckpt_dir}")
...
def main(args=None) -> None:
    ...
    node = RealtimeHermesHiPolicyNode()
    try:
