#!/usr/bin/env python3
"""Real-time MuJoCo CPU position-control teleop (keyboard -> command -> policy).

This node is intended for position-controlled joystick policies (e.g. older
Phonebot FV2 position-control policies, Chopstickbot joystick policies).

- Policy outputs actions in [-1, 1] (typical PPO tanh_normal).
- We convert actions -> position targets (motor_targets) and write directly to
  `mjData.ctrl[:] = motor_targets`.
"""
...
class RealtimeMujocoPositionNode(Node):
    """Run MuJoCo CPU sim and drive a position-controlled policy in real time."""
    ...
    def __init__(self) -> None:
        super().__init__("realtime_mujoco_position")
        ...
        self.declare_parameter(
            "env_name", "PhonebotJoystickFlatTerrainAlterFV2"
        )
        self.declare_parameter("task", "flat_terrain_alternative_imu_fv2")
        ...
        # If this env supports explicit XML overrides, keep MJX env consistent
        # with the MuJoCo CPU model (useful for checkpoint restore / normalizer).
        if xml_path_override:
            try:
                env_cfg.xml_path_override = str(xml_path_override)
            except Exception:
                pass
    ...
    def _on_timer_unlocked(self) -> None:
        ...
        # Position control: write targets directly to ctrl.
...
def main(args=None) -> None:
    ...
    node = RealtimeMujocoPositionNode()
    try:
