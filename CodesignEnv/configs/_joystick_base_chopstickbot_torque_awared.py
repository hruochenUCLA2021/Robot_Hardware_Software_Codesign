...
from .motor_control import TorqueLimitedPDController, TorqueLimitedPDParams
...
def default_config() -> config_dict.ConfigDict:
  return config_dict.create(
      ...
      motor_params_path=os.path.join(os.path.dirname(__file__), "chopstickbot_motor_defaults.yaml"),
      ...
      reward_config=config_dict.create(
          scales=config_dict.create(
              ...
              # I think this should be all negative as it is penalty 
              torques=-0.1,
              torques_square=-0.01,
              energy=0.0,
              energy_square=-0.01,
...
class BaseJoystick(hi_base.HiEnv):
  ...
  def _post_init(self) -> None:
    # Initialize from the configured keyframe if present, otherwise fall back to qpos0.
    keyframe_name = str(getattr(self._config, "home_keyframe_name", "home"))
    try:
      keyframe = self._mj_model.keyframe(keyframe_name)
    ...
    # Torque-limited motor controller (ToddlerBot-style).
    self._motor_controller = self._build_motor_controller()

  def _build_motor_controller(self) -> TorqueLimitedPDController:
    ...
    kp_ratio = float(cfg.get("kp_ratio", 150.0))
    kd_ratio = float(cfg.get("kd_ratio", 16.0))
    passive_active_ratio = float(cfg.get("passive_active_ratio", 3.0))
    m = cfg.get("motor", {}) or {}

    kp = float(m.get("kp_real", 600.0)) / kp_ratio
    kd = float(m.get("kd_real", 0.0)) / kd_ratio

    n = int(self.mjx_model.nu)
    arr = lambda x: jp.ones((n,), dtype=jp.float32) * jp.asarray(x, dtype=jp.float32)
    p = TorqueLimitedPDParams(
        kp=arr(kp),
        kd=arr(kd),
        kd_min=arr(float(m.get("kd_min", 0.162))),
        passive_active_ratio=passive_active_ratio,
        tau_max=arr(float(m.get("tau_max", 0.93))),
        q_dot_tau_max=arr(float(m.get("q_dot_tau_max", 2.0))),
        q_dot_max=arr(float(m.get("q_dot_max", 5.97))),
        tau_q_dot_max=arr(float(m.get("tau_q_dot_max", 0.08))),
        tau_brake_max=arr(float(m.get("tau_brake_max", 1.4))),
    )
  ...
  def reset(self, rng: jax.Array) -> mjx_env.State:
    ...
    # Torque actuators: initialise ctrl to 0 Nm.
    data = mjx_env.init(
        self.mjx_model, qpos=qpos, qvel=qvel, ctrl=jp.zeros((self.mjx_model.nu,), dtype=jp.float32)
    )
  ...
  def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
    ...
    # Convert targets to torque via torque-limited PD controller.
    q = state.data.qpos[7:]
  ...
  def _cost_energy_square(self, qvel: jax.Array, actuator_force: jax.Array) -> jax.Array:
    n = min(qvel.shape[0], actuator_force.shape[0])
    return jp.mean(jp.square(qvel[:n] * actuator_force[:n]))
  ...
  def _get_reward(
    ...
    energy_sq_cost = self._cost_energy_square(joint_qvel, data.actuator_force)
