...
def default_config() -> config_dict.ConfigDict:
  return config_dict.create(
      ...
      lin_vel_y=[-0.25, 0.25],
      ang_vel_yaw=[-1.0, 1.0],
      # Morphology conditioning (used by the policy observation).
      morphology=config_dict.create(
...
class BaseJoystick(hi_base.HiEnv):
  ...
  def _get_obs(self, data: mjx.Data, info: dict[str, Any], contact: jax.Array) -> dict[str, jax.Array]:
    ...
    # Morphology input: normalized leg length in [-1, 1].
    mcfg = getattr(self._config, "morphology", None)
    if mcfg is None:
      leg_len_norm = jp.asarray(0.0, dtype=jp.float32)
    else:
      L = jp.asarray(getattr(mcfg, "leg_length_m", 0.0), dtype=jp.float32)
      rng = getattr(mcfg, "leg_length_range_m", [0.2, 5.0])
      lo = jp.asarray(float(rng[0]), dtype=jp.float32)
      hi = jp.asarray(float(rng[1]), dtype=jp.float32)
      denom = jp.maximum(hi - lo, 1e-6)
      leg_len_norm = (2.0 * (L - lo) / denom) - 1.0
