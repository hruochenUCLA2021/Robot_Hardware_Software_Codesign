...
def domain_randomize(model: mjx.Model, rng: jax.Array):
  ...
  def rand_dynamics(key):
    ...
    k_fric, k_floss, k_arm, k_mass, k_qpos0, k_gear, k_kd = jax.random.split(key, 7)
    ...
    # -----------------------------------------------------------------------
    # Torque actuators (<motor>): scale actuator_gainprm[:, 0] which behaves like
    # the motor "gear"/torque scale for MJX. Do NOT touch biasprm (unused by motor).
    # -----------------------------------------------------------------------
    gear_scale = (
        _u(k_gear, shape=(int(model.nu),), lo=0.9, hi=1.1) if int(model.nu) > 0 else jp.zeros((0,))
    )
    actuator_gainprm = model.actuator_gainprm
    if int(model.nu) > 0:
      gear = model.actuator_gainprm[:, 0] * gear_scale
      actuator_gainprm = actuator_gainprm.at[:, 0].set(gear)
  ...
  in_axes = in_axes.tree_replace(
      {
          ...
          "qpos0": 0,
          "actuator_gainprm": 0,
          # actuator_biasprm is not randomized for <motor> actuators.
      }
  )
