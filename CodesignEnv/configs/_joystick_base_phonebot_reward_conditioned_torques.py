...
def default_config() -> config_dict.ConfigDict:
  return config_dict.create(
      ...
      # Reward-conditioning: per-actuator penalty weights (fed to policy obs).
      #
      # `weights` are sampled in `weights_range` (default [0, 1]):
      # - higher value => higher penalty for that actuator
      # - 0.0 => no penalty for that actuator
      #
      # The base reward scales `reward_config.scales.torques` and
      # `reward_config.scales.torques_square` still apply, but the *cost* is
      # weighted per-actuator by these sampled weights.
...
class BaseJoystick(hi_base.HiEnv):
  ...
  def _sample_torque_weights(self, rng: jax.Array) -> jax.Array:
    cfg = self._config.torque_weight_config
    lo = float(cfg.weights_range[0])
    hi = float(cfg.weights_range[1])
    return jax.random.uniform(rng, (self.mjx_model.nu,), minval=lo, maxval=hi)
  ...
  def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
    ...
    # Resample reward-conditioning weights periodically (optional).
    tw_cfg = self._config.torque_weight_config
    state.info["torque_w_step"] = state.info["torque_w_step"] + 1
    state.info["rng"], tw_rng = jax.random.split(state.info["rng"])
    do_resample = (
        tw_cfg.enable
  ...
  def _torque_weight_factor(self, info: dict[str, Any], torques: jax.Array) -> jax.Array:
    """Returns per-actuator weights in [0, +inf) aligned to `torques` length."""
    n = min(torques.shape[0], info["torque_w"].shape[0])
    # Higher value => higher penalty. (Range is controlled by sampling config.)
    w = info["torque_w"][:n]
    w = w * jp.asarray(self._config.torque_weight_config.global_scale, dtype=torques.dtype)
    return w

  def _cost_torques(self, torques: jax.Array, info: dict[str, Any]) -> jax.Array:
    n = min(torques.shape[0], info["torque_w"].shape[0])
    w = self._torque_weight_factor(info, torques)
    return jp.sum(jp.abs(torques[:n]) * w)
  ...
  def _cost_torques_square(self, torques: jax.Array, info: dict[str, Any]) -> jax.Array:
    n = min(torques.shape[0], info["torque_w"].shape[0])
    w = self._torque_weight_factor(info, torques)
    return jp.mean(jp.square(torques[:n]) * w)
  ...
  def _get_reward(
    ...
    torques_cost = self._cost_torques(data.actuator_force, info)
    torques_sq_cost = self._cost_torques_square(data.actuator_force, info)
