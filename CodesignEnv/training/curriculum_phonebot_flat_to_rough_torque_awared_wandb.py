#!/usr/bin/env python3
"""
Curriculum Learning (with W&B): PhonebotJoystickFlatTerrain → PhonebotJoystickRoughTerrain

This script implements a two-stage curriculum for PhoneBot:
...
def get_ppo_params_for_phonebot() -> config_dict.ConfigDict:
...
def train_stage(
  ...
  """Train a single stage (flat or rough) of the PhoneBot joystick curriculum."""
  _configure_jax()
  ...
  if is_torque_awared:
    if not is_alt_fv2:
      raise ValueError("TorqueAwared envs currently expect AlterFV2 task naming.")
    task = f"{task}_alternative_imu_fv2_torque"
  else:
    if is_alt_fv2:
  ...
  # Pass overrides into env constructor too (so HiEnv merges into ConfigDict).
  env = EnvClass(task=task, config=env_cfg, config_overrides=env_config_overrides)
  eval_env = EnvClass(task=task, config=env_cfg, config_overrides=env_config_overrides)
  print(f"Environment created: {type(env).__name__}")
  print(f"Action size: {env.action_size}")

  # PPO hyperparameters with local overrides.
  ppo_params = get_ppo_params_for_phonebot()
  ...
  try:
    wandb_run = wandb.init(
        project="PhonebotJoystickCurriculum",
...
def main():
  """Run PhoneBot joystick flat → rough curriculum with 100M steps each."""
