...
from CodesignEnv.training import (  # noqa: E402
    curriculum_phonebot_flat_to_rough_torque_awared_wandb as curriculum_trainer,
)
...
def main():
  ...
  config_path = os.path.join(_THIS_DIR, "train_phonebot_joystick_torque_awared_config.yaml")
  if not os.path.exists(config_path):
    raise FileNotFoundError(
        f"train config not found at: {config_path}. "
        "Please create it before running train_phonebot_torque_awared_joystick.py."
    )
  ...
  if not env_name_flat:
    if use_alt_imu_fv2:
      env_name_flat = "PhonebotJoystickFlatTerrainAlterFV2TorqueAwared"
    else:
  ...
  if not env_name_rough:
    if use_alt_imu_fv2:
      env_name_rough = "PhonebotJoystickRoughTerrainAlterFV2TorqueAwared"
    else:
