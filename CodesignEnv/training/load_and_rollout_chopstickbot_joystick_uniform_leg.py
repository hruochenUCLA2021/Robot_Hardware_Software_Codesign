...
def main():
  ...
  cfg_path = sys.argv[1] if len(sys.argv) > 1 else os.path.join(
      _THIS_DIR, "rollout_chopstickbot_joystick_config_uniform_leg.yaml"
  )
  ...
  # Optional per-rollout overrides for morphology-conditioned envs.
  xml_path_override = str(cfg.get("xml_path_override", "") or "").strip()
  leg_length_m = cfg.get("leg_length_m", None)
  ...
  if xml_path_override:
    env_overrides["xml_path_override"] = xml_path_override
  if leg_length_m is not None:
    # Nested config (ConfigDict) update supports dict-of-dicts.
    env_overrides["morphology"] = {"leg_length_m": float(leg_length_m)}
