...
# Use the torque-aware stage trainer.
from CodesignEnv.training import (  # noqa: E402
    curriculum_phonebot_flat_to_rough_torque_awared_ankle_collision_wandb as curriculum_trainer,
)
...
def main():
  ...
  config_path = os.path.join(_THIS_DIR, "train_phonebot_joystick_torque_awared_ankle_collision_config.yaml")
  if not os.path.exists(config_path):
    raise FileNotFoundError(
        f"train config not found at: {config_path}. "
        "Please create it before running train_phonebot_torque_awared_joystick_ankle_collision.py."
    )
  ...
  home_keyframe_name_default = str(env_section.get("home_keyframe_name", "home"))
  env_name_flat = env_section.get("env_name_flat")
  env_name_rough = env_section.get("env_name_rough")
  if not env_name_flat:
    if use_alt_imu_fv2:
      env_name_flat = "PhonebotJoystickFlatTerrainAlterFV2TorqueAwaredAnkleCollision"
    else:
      # Fallback: keep legacy names if requested, but torque-aware training is
      # primarily intended for AlterFV2TorqueAwared envs.
      env_name_flat = "PhonebotJoystickFlatTerrainAlter" if use_alt_imu else "PhonebotJoystickFlatTerrain"
  if not env_name_rough:
    if use_alt_imu_fv2:
      env_name_rough = "PhonebotJoystickRoughTerrainAlterFV2TorqueAwaredAnkleCollision"
    else:
  ...
  if mode == "flat":
    ...
    home_keyframe_name = str(flat_cfg.get("home_keyframe_name", home_keyframe_name_default))
    ...
    curriculum_trainer.train_stage(
        ...
        env_config_overrides={"home_keyframe_name": home_keyframe_name},
    )
    ...
    restore = epath.Path(ckpt) if ckpt else None
    home_keyframe_name = str(rough_cfg.get("home_keyframe_name", home_keyframe_name_default))
    ...
    flat_home_keyframe_name = str(cur_cfg.get("flat_home_keyframe_name", home_keyframe_name_default))
    flat_final_ckpt = curriculum_trainer.train_stage(
        ...
        env_config_path=env_cfg_path,
        env_config_overrides={"home_keyframe_name": flat_home_keyframe_name},
    )
    ...
    rough_home_keyframe_name = str(cur_cfg.get("rough_home_keyframe_name", home_keyframe_name_default))
    curriculum_trainer.train_stage(
        env_name=str(env_name_rough),
        stage_name="rough",
        ...
        env_config_path=env_cfg_path,
        env_config_overrides={"home_keyframe_name": rough_home_keyframe_name},
    )
