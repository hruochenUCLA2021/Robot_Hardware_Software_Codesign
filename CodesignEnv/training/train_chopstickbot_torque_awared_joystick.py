...
from CodesignEnv.training import (  # noqa: E402
    curriculum_chopstickbot_flat_to_rough_torque_awared_wandb as curriculum_trainer,
)
...
def main():
  ...
  config_path = os.path.join(_THIS_DIR, "train_chopstickbot_joystick_torque_awared_config.yaml")
  if not os.path.exists(config_path):
    raise FileNotFoundError(
        f"train config not found at: {config_path}. "
        "Please create it before running train_chopstickbot_torque_awared_joystick.py."
    )
  ...
  if not env_name_flat:
    env_name_flat = "ChopstickbotJoystickFlatTerrainAlterTorqueAwared" if use_alt_imu else "ChopstickbotJoystickFlatTerrain"
  if not env_name_rough:
    env_name_rough = "ChopstickbotJoystickRoughTerrainAlterTorqueAwared" if use_alt_imu else "ChopstickbotJoystickRoughTerrain"
