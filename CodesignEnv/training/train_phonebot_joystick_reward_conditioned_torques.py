...
def main():
  ...
  config_path = os.path.join(_THIS_DIR, "train_phonebot_joystick_config_reward_conditioned_torques.yaml")
  ...
  if not env_name_flat:
    if use_alt_imu_fv2:
      env_name_flat = "PhonebotJoystickFlatTerrainAlterFV2RewardConditionedTorques"
    else:
      env_name_flat = "PhonebotJoystickFlatTerrainAlter" if use_alt_imu else "PhonebotJoystickFlatTerrain"
  if not env_name_rough:
    if use_alt_imu_fv2:
      env_name_rough = "PhonebotJoystickRoughTerrainAlterFV2RewardConditionedTorques"
    else:
