#!/usr/bin/env python3
"""
Load and rollout ChopstickbotJoystick* policy.

This script:
- Loads a trained Chopstickbot joystick policy from a checkpoint directory.
...
def load_policy_from_checkpoint(
    ckpt_dir: epath.Path,
    env_name: str = "ChopstickbotJoystickFlatTerrain",
  ...
  if is_torque_awared:
    if not is_alt:
      raise ValueError("TorqueAwared envs currently expect Alter task naming.")
    task = f"{task}_alternative_imu_torque"
  else:
    if is_alt:
...
def rollout_joystick(
    ...
    env_registry_name: str | None = None,
):
  ...
  # See PhoneBot rollout script for details.
  # -------------------------------------------------------------------------
  ...
  if record_json is not None:
    ...
    record_data = {
        ...
        "env_class": str(type(env).__name__),
        "xml_path": getattr(env, "xml_path", None),
        "home_keyframe_name": str(getattr(env_cfg, "home_keyframe_name", "home")),
        "motor_params_path": str(getattr(env_cfg, "motor_params_path", "")),
...
def main():
  ...
  env_name = cfg.get("env_name", "ChopstickbotJoystickFlatTerrain")
  ...
  env_overrides = None
  if isinstance(cfg, dict) and ("home_keyframe_name" in cfg):
    env_overrides = {"home_keyframe_name": str(cfg.get("home_keyframe_name"))}
  ...
  output_dir = cfg.get("output_dir", "video_rollout_chopstickbot")
